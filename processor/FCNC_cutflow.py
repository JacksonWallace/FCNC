...
fileset = make_fileset(['hut', 'hct'], year, redirector=redirector_ucsd, small=False)
...
class FCNC_cutflow(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        tight = (((ak.num(lepton) == 2) & leading)) #& (ak.sum(lepton.charge, axis=1) != 0)) | (ak.num(lepton) > 2))
        ...
        g_mr_veto = ( ( (ak.num(loose_lepton) == 3) & ( (ak.all(np.abs(diloose_electron_OS.mass) > 12, axis=1)) & (ak.all(np.abs(diloose_muon_OS.mass) > 12, axis=1)) )  )  | (ak.num(loose_lepton) == 2) )
        first_ss = SSlepton
        ss = (SSlepton & gmass & Z_mr_veto & g_mr_veto & leading)
        ...
        #ML SRs
        gmass2 = (((ak.num(lepton) >= 2) & (ak.all(dilepton_mass > 12, axis=1))) | (ak.num(lepton) < 2) )
        ...
        ml =  ( (ak.num(lepton) > 2) & gmass2 & Z_mr_veto2)
        ...
        selection.add('two tight',         tight)
        selection.add('ss',                first_ss)
        selection.add('hyp4',              ss)
        ...
        cf_ss_reqs = ['"skim"'] + ['MET > 50'] + ['two tight'] + ['ss'] + ['hyp4'] + ['two jets'] + ['triggers'] + ['filter'] + ['SS onZ veto']
        
        bl_reqs = sk_reqs + ['lepton selection'] + ['leading lepton'] + ['triggers'] + ['filter'] + ['MET > 50']
        bl_reqs_d = { sel: True for sel in bl_reqs }
        baseline = selection.require(**bl_reqs_d)
        
        ss_reqs = bl_reqs + ['ss'] + ['SS onZ veto'] + ['two jets'] 
        ss_reqs_d = { sel: True for sel in ss_reqs }
        ss_sel = selection.require(**ss_reqs_d)
        
        ml_reqs = bl_reqs + ['multilep'] + ['one jet']
        ...
        output["j_vs_b_ss_flips"].fill(                          #flips
            dataset = dataset,
            n1 = ak.num(jet[flip_sel]),
            n2 = ak.num(btag[flip_sel]),
            weight = weight2.weight()[flip_sel]
        )
        
        output["j_vs_b_ss_fakes"].fill(                          #fakes
            dataset = dataset,
            n1 = ak.num(jet[fake_ss_sel]),
            n2 = ak.num(btag[fake_ss_sel]),
            weight = weight2.weight()[fake_ss_sel]
        )
        
        output["j_vs_b_ss_non_fakes_flips"].fill(                #rares
            dataset = dataset,
            n1 = ak.num(jet[ss_sel & no_fake & no_flip]),
            n2 = ak.num(btag[ss_sel & no_fake & no_flip]),
            weight = weight2.weight()[ss_sel & no_fake & no_flip]
        )
        
        
        output["j_vs_b_ml"].fill(                                #signal
            dataset = dataset,
            n1 = ak.num(jet[ml_sel]),
            n2 = ak.num(btag[ml_sel]),
            weight = weight2.weight()[ml_sel]
        )
        
        output["j_vs_b_ml_fakes"].fill(                          #fakes
            dataset = dataset,
            n1 = ak.num(jet[fake_ml_sel]),
            n2 = ak.num(btag[fake_ml_sel]),
            weight = weight2.weight()[fake_ml_sel]
        )
        
        output["j_vs_b_ml_non_fakes"].fill(                      #rares
            dataset = dataset,
            n1 = ak.num(jet[ml_sel & no_fake]),
            n2 = ak.num(btag[ml_sel & no_fake]),
            weight = weight2.weight()[ml_sel & no_fake]
        )
